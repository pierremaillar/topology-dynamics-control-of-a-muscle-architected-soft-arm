...
class MemoryBlockMuscularRod(
    ...
    def __init__(self, systems: Sequence, system_idx_list):
        ...
        _reset_scalar_ghost(self.force_velocity_constant, self.ghost_elems_idx, 1.0)
    ...
    def allocate_block_variables_in_elements(self, systems: Sequence):
        ...
        # Things in elements that are  9 dim vectors
        # These are mostly
        ...
        map_nine_dim_vector_dofs_in_rod_elems = {
            # "normalized_active_force_y_intercept": 0,
            # "normalized_active_force_slope": 1,
            # "normalized_active_force_break_points": 2,
            "active_force_coefficients": 0,
            "passive_force_coefficients": 1,
        }
        self.nine_dim_vector_dofs_in_rod_elems = np.zeros(
            (len(map_nine_dim_vector_dofs_in_rod_elems), 9 * self.n_elems)
        )
        for k, v in map_nine_dim_vector_dofs_in_rod_elems.items():
            self.__dict__[k] = np.lib.stride_tricks.as_strided(
                self.nine_dim_vector_dofs_in_rod_elems[v], (9, self.n_elems)
            )
        ...
        #
        # for k, v in map_three_dim_vector_dofs_in_rod_elems.items():
        #     for system_idx, system in enumerate(systems):
        #         start_idx = self.start_idx_in_rod_elems[system_idx]
        #         end_idx = self.end_idx_in_rod_elems[system_idx]
        #         self.__dict__[k][..., start_idx:end_idx] = system.__dict__[k].copy()
