__doc__ = """Create block-structure class for collection of Linear 1D ribbon."""
import numpy as np
...
from elastica._elastica_numba._rod._ribbon1D import (
    _compute_sigma_kappa_for_blockstructure,
)


class MemoryBlockLinearRibbon1D(
    MemoryBlockRodBase, LinearRibbon1D, _RodSymplecticStepperMixin
):
    """
    Memory block class for 1D ribbon equations. This class is derived from Cosserat Rod class in order to inherit
    the methods of Cosserat rod class. This class takes the Ribbon1D object (systems) and creates big
    arrays to store the system data and returns a reference of that data to the systems.
    Thus each system is now in contiguous memory, so it is faster to compute 1D Ribbon equations.
