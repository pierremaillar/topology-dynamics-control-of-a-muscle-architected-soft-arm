...
class Environment:
    ...
    def reset(self):
        ...
        for idx, rod in enumerate(self.straight_rod_list):
            ...
            #                self.simulator.add_forcing_to(rod).using(SigmoidActivationLongitudinalMuscles, start_time = 3-80*0.007, end_time = 3, start_idx = 0, end_idx=170, beta=1, tau=0.007, activation_level_max=0.8, activation_level_end=0.00)
        #            if idx > 0:
        #                self.simulator.add_forcing_to(rod).using(
        #                    SigmoidActivationLongitudinalMuscles,
        #                    start_time=3.1,
        #                    end_time=13.1 + 40 * 0.0055,
        #                    start_idx=0,
        #                    end_idx=170,
        #                    beta=0.6,
        #                    tau=0.0055,
        #                    activation_level_max=0.1,
        #                    activation_level_end=0.00,
        #                )
        #            else:
        #                self.simulator.add_forcing_to(rod).using(SigmoidActivationLongitudinalMuscles, start_time = 3.05-0*0.005, end_time = 13.05+0*0.005, start_idx = 0, end_idx=170, beta=1, tau=0.007, activation_level_max=0.1, activation_level_end=0.00)
        ...
        #                    tau=0.0055,
        #                    activation_level_max=0.1,
        #                    activation_level_end=0.1,
        #                )  # 1.0
        #            elif idx >= 150 and idx <= 170:
        #                self.simulator.add_forcing_to(rod).using(
        #                    SigmoidActivationTransverseMuscles,
        #                    rod_idx=idx,
        #                    start_time=3.1,
        ...
        #                    beta=0.6,
        #                    tau=0.0055,
        #                    activation_level_max=0.05,
        #                    activation_level_end=0.05,
        #                )  # 0.5
        #            else:
        #                self.simulator.add_forcing_to(rod).using(
        #                    SigmoidActivationTransverseMuscles,
        ...
        #                    beta=0.6,
        #                    tau=0.0055,
        #                    activation_level_max=0.025,
        #                    activation_level_end=0.025,
        #                )  # 0.5
        ...
        #        for idx, rod in enumerate(self.helical_rod_list[:4]):
        ##            self.simulator.add_forcing_to(rod).using(
        ##                    LocalActivation, ramp_interval = 2.0, ramp_up_time=0.0, ramp_down_time = 5.0, start_idx = 0, end_idx = 160, activation_level=0.30,
        ##                )
        #            self.simulator.add_forcing_to(rod).using(SigmoidActivationLongitudinalMuscles, start_time = 3-160*0.005, end_time = 3, start_idx = 0, end_idx=359-18, beta=1, tau=0.005, activation_level_max=0.30, activation_level_end=0.0)
        ...
        # Drag force
        rho_water = 1.050e-3  # g/mm3
        # Surface area scaling Asurface LM leading edge by arm
